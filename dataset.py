...
def read_data_file(train_path, c):
    ...
    train_PC9_label = [1] * len(train_PC9_2d)
    train_PC9GR_2d = [[0] * c for _ in range(len(train_PC9GR_list) // c)]
    train_PC9GR_label = [0] * len(train_PC9GR_2d)
    ...
    for i, (pc9_name, pcGR_name) in enumerate(zip(train_PC9_list, train_PC9GR_list)):
        ...
        train_PC9_2d[row][column] = pc9_name
        train_PC9GR_2d[row][column] = pcGR_name

    # combine the p and n data
    train_set = train_PC9_2d.extend(train_PC9GR_2d)
    train_set_label = train_PC9_label.extend(train_PC9GR_label)

    return train_set, train_set_label

def get_bacth_data(train_path, img_size, channel, batch_size):
