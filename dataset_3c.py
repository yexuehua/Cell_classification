...
def read_data_file(data_path, c):
    ...
    data_path_others = os.path.join(data_path, "others")
    data_PC9_list = os.listdir(data_path_PC9)
    data_PC9GR_list = os.listdir(data_path_PC9GR)
    data_others_list = os.listdir(data_path_others)

    # create lists to store filename and label
    data_PC9_2d = [[0] * c for _ in range(len(data_PC9_list) // c)]
    data_PC9_label = [[1, 0, 0] for _ in range(len(data_PC9_2d))]
    data_PC9GR_2d = [[0] * c for _ in range(len(data_PC9GR_list) // c)]
    data_PC9GR_label = [[0, 1, 0] for _ in range(len(data_PC9GR_2d))]
    data_others_2d = [[0] * c for _ in range(len(data_others_list) // c)]
    data_others_label = [[0, 0, 1] for _ in range(len(data_others_2d))]
    # add the filenames of channels to a row
    for i, (pc9_name, pcGR_name, others_name) in enumerate(zip(data_PC9_list, data_PC9GR_list, data_others_list)):
        row, column = divmod(i, c)
        data_PC9_2d[row][column] = "PC9/"+pc9_name
        data_PC9GR_2d[row][column] = "PC9GR/"+pcGR_name
        data_others_2d[row][column] = "others/" + others_name

    # combine the p and n data
    data_PC9_2d.extend(data_PC9GR_2d)
    data_PC9_2d.extend(data_others_2d)
    data_PC9_label.extend(data_PC9GR_label)
    data_PC9_label.extend(data_others_label)
